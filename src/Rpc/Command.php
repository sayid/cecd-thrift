...
class Command
{
     ...
     * @return mixed
     */
    public static function main($root, $argv)
    {
        $command = new static();
        $command->root = $root;
    ...
    public function run($argv)
    {
        ...
        $this->serverOptions['log_file'] = $root .'storage/logs/swoole.log';

        $handler = new RpcServiceHandle();
        $processor = new \Ce\Sdk\Rpc\Base\RpcServiceProcessor($handler);

        $socket_tranport = new TSwooleServerTransport('0.0.0.0', 8192);
        ...
        $server->serve($this->host, $this->port);
    ...
    public function usage()
    {
        ...
        echo <<<'EOT'
Usage: vendor/bin/cerpc-swoole {stop|restart|reload}
